...
help="list all the snapshots, even the older ones")
...
def create_snapshots(project):
    "create snapshot for EC2 volumes"
    ...
    for i in instances:
        ...
        for v in i.volumes.all():
            if has_pending_snapshot(v):
                print("  Skipping the {0}..already in progress" .format(v.id))
                continue
...
def stop_instances(project):
    ...
    for i in instances:
        ...
        try:
            i.stop()
        except botocore.exceptions.ClintError as e:
